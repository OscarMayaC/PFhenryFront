...
export var useFieldCharacterEditing = function useFieldCharacterEditing(_ref) {
  ...
  var applyLetterEditing = function applyLetterEditing(params) {
    ...
    var testQueryOnFormatAndFallbackFormat = function testQueryOnFormatAndFallbackFormat(queryValue, activeSection, fallbackFormat, formatFallbackValue) {
      var getOptions = function getOptions(format) {
        return getLetterEditingOptions(utils, activeSection.type, format);
      };
  ...
  var applyNumericEditing = function applyNumericEditing(params) {
    var getNewSectionValue = function getNewSectionValue(queryValue, sectionType, format, hasLeadingZeros, contentType) {
      var queryValueNumber = Number("".concat(queryValue));
      var sectionBoundaries = sectionsValueBoundaries[sectionType]({
        currentDate: null,
        format: format,
      ...
      var newSectionValue = cleanDigitSectionValue(utils, queryValueNumber, sectionType, format, hasLeadingZeros, sectionBoundaries);
      return {
    ...
    var getFirstSectionValueMatchingWithQuery = function getFirstSectionValueMatchingWithQuery(queryValue, activeSection) {
      ...
      if (activeSection.type === 'month') {
        var response = getNewSectionValue(queryValue, activeSection.type, 'MM', doesSectionHaveLeadingZeros(utils, 'digit', 'month', 'MM'), 'digit');
        if (isQueryResponseWithoutValue(response)) {
          return response;
        }
      ...
      if (activeSection.type === 'weekDay') {
        var _response = getNewSectionValue(queryValue, activeSection.type, activeSection.format, activeSection.hasLeadingZeros, activeSection.contentType);
        if (isQueryResponseWithoutValue(_response)) {
          return _response;
        }
        var _formattedValue = getDaysInWeekStr(utils, activeSection.format)[Number(_response.sectionValue) - 1];
